...
public class SearchTestsPageObjectSelenide {
    ...
    public static void cartVerification(){
        ...
        String bookName = HomePageSelenide.getFirstBookName();
        String bookPrice1 = HomePageSelenide.getFirstBookPrice();
        HomePageSelenide.selectBookWithPrice();

        String newBookName = BookDetailedPageSelenide.getBookName();
        Assert.assertEquals(bookName, newBookName, "Book names should be the same");

        String classNameBefore = BookDetailedPageSelenide.getCartStateClassName();
        BookDetailedPageSelenide.addToCart();

        String classNameAfter = BookDetailedPageSelenide.getCartStateClassName();

        Assert.assertNotEquals(classNameBefore, classNameAfter, "Class names should be different");

        String qtyInCart = BookDetailedPageSelenide.getQtyInCart();
        Assert.assertTrue(qtyInCart.endsWith("1"), "Should be one book in the cart");

        String bookPrice2 = BookDetailedPageSelenide.getBookPrice();
        Assert.assertEquals(bookPrice1, bookPrice2, "Prices should be the same");

        BookDetailedPageSelenide.clickPromoCodeButton();

        BookDetailedPageSelenide.sendPromoCode("813524");

        boolean isDiscountBannerDisplayed = BookDetailedPageSelenide.isDiscountBannerDisplayed();
        Assert.assertTrue(isDiscountBannerDisplayed, "Discount banner should be displayed");

        int discountRate = BookDetailedPageSelenide.getDiscountRate();

        float oldPriceValue = BookDetailedPageSelenide.getOldPrice();
        float newPriceValue = BookDetailedPageSelenide.getNewPrice();
...
public void sendMessage(){
    HomePageSelenide.sendMessage("Hello!");
